...
void Double_linked_list::insert(Node2* ite, int data) {
	if (ite == tail) {
		this->append(data);
	}
	Node2* new_node = new Node2(data);
	new_node->next = ite->next;
	ite->next->prev = new_node;
	ite->next = new_node;
	new_node->prev = ite;
	++size;
}
...
Node2* Double_linked_list::remove(Node2* ite) {
	if (ite == tail) {
		tail = tail->prev;
		tail->next = NULL;
		--size;
...
void Double_linked_list::Insertion_sort() {
	Node2* ite1 = head->next;
	while(ite1 != NULL) {
		Node2* ite2 = ite1->prev;
		int insert_val = ite1->data;
		while (ite2!=NULL && ite2->data > insert_val) {
			ite2 = ite2->prev;
		}
		if (ite2 != NULL) {
			this->insert(ite2, insert_val);
			ite1 = this->remove(ite1);
		}
		if (ite2 == NULL) {
			this->prepend(insert_val);
			ite1 = this->remove(ite1);
		}
	}
}
